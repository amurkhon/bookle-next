...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
		...
		GET_BOARD_ARTICLE,
		{
			...
			onCompleted: (data: T) => {
				...
				if(data?.getBoardArticle?.memberData?.memberImage) {
					setMemberImage(`${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/${data?.getBoardArticle?.memberData?.memberImage}`);
				}
			},
		}
	);
	...
	const createCommentHandler = async () => {
		...
		try {
			...
			await getArticleRefetch({input: articleId});
			setComment('');
			sweetMixinSuccessAlert('Successfully commented!');
		} catch(err: any) {
	...
	const getCommentMemberImage = (imageUrl: string | undefined) => {
		if (imageUrl) return `${process.env.NEXT_PUBLIC_REACT_APP_API_URL}/${imageUrl}`;
					...
					<Stack className="main-box">
						<Stack className="left-config">
							<Stack className={'image-info'}>
								<img src={'/img/logo/vendor.jpg'} />
								<Stack className={'community-name'}>
									<Typography className={'name'}>Bookle Community</Typography>
								</Stack>
							</Stack>
							<Tabs
									...
									<Stack className="content-and-info">
										...
										<Stack className="info">
											<Stack className="icon-info">
												{boardArticle?.meLiked?.length === 0 ? <ThumbUpOffAltIcon onClick={()=>likeBoardArticleHandler(user, boardArticle?._id)} /> : <ThumbUpAltIcon onClick={()=>likeBoardArticleHandler(user, boardArticle?._id)}  />}
